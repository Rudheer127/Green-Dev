...
function MiniNavbar() {
  ...
  const loginButtonElement = (
    <a href="#how-it-works" className="px-4 py-2 sm:px-3 text-xs sm:text-sm border border-[#333] bg-[rgba(31,31,31,0.62)] text-gray-300 rounded-full hover:border-white/50 hover:text-white transition-colors duration-200 w-full sm:w-auto text-center">
      How it Works
    </a>
  );
...
export const SignInPage = ({ className }: SignInPageProps) => {
  ...
  const skipCodeShortcut = () => {
      // Just a mock hack to jump to dashboard without filling code
      router.push('/start');
  };
                ...
                {step === "email" ? (
                  ...
                  >
                    ...
                    <div className="space-y-4">
                      <button onClick={skipCodeShortcut} className="backdrop-blur-[2px] w-full flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 text-white border border-white/10 rounded-full py-3 px-4 transition-colors">
                        <svg className="w-4 h-4" viewBox="0 0 28 28" fill="none"><path d="M14 3C8 3 4 8 4 14c0 4 2.5 7.5 6 9.5L14 25l4-2.5c3.5-2 6-5.5 6-9.5 0-6-4-11-10-11z" fill="white" opacity="0.3"/><path d="M8 14c0-3.5 2.5-6.5 6-7.5M14 22c3.5-1 6-4 6-8" stroke="white" strokeWidth="1.5" strokeLinecap="round"/></svg>
                        <span>Continue with GreenDev</span>
                      </button>
